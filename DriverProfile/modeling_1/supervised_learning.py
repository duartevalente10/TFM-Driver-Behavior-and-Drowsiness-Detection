...
if __name__ == "__main__":
    ...
    # read dataset
    df = pd.read_csv('../datasets/supervised/trips_kmeans_no_reduction_distance.csv')
    ...
    print('------------------ Feature Selection and Reduction ------------------ \n')
    ...
    # RRFS
    #features = relevance_redundancy_filter(data, 'MM', 'AC', 6, 0.5)
    ...
    features = relevance_redundancy_filter(df, 'FR', 'AC', 6, 0.5)
    ...
    for a in algs:
        ...
        path = '../modeling/images/supervised/{}/best_resample/multiclass/smote_'.format(a)
        cross_validation(X_train, y_train, pipeline, cv=cv, is_multi_class=True, path=path)
        estim_path = '../modeling/models/{}_model_pca_multi'.format(a)
        find_best_estimator(X_train, y_train, pipeline, grids[a], cv=parameter_cv, is_multi_class=True, path=estim_path, verbose=2)
